...
export function isErrorCode(value: string): value is ErrorCode {
  return (
    ...
    ErrorCode.FeatureNotEnabled === value
  );
}
...
export const defaultStatusMessages: Record<ErrorCode, string> = {
  ...
  [ErrorCode.FeatureNotEnabled]: "You do not have access to this feature",
...
export class InteractionError extends Error {
  ...
  constructor(message: string | ErrorCode | StatusMessage) {
    if (typeof message === "string") {
      super(isErrorCode(message) ? defaultStatusMessages[message] : message);
